 ...
 *
 * <p>The output file format is intended to be human-readable, with indented JSON produced
 * by enabling {@link SerializationFeature#INDENT_OUTPUT}. The file extension used for saved
 * games is {@code .millions}, matched by {@link DataReaderFactory#getGameReader(String)}
 * on load.</p>
 */
public class JsonGameWriter {
   ...
   *
   * <p>If a file already exists at {@code destination}, it will be overwritten.</p>
   *
   * @param destination the absolute or relative path to write the JSON file to
   * @param state       the game state data transfer object to serialise; must not be
   *                    {@code null}

...
/**
 * Tab view listing all available market stocks with a live search filter and per-row buy buttons.
 *
 * <p>The layout is arranged vertically:
 *
 * <ol>
 *   <li>A top container with a title, subtitle, and a search {@link TextField} that filters the
 *       table in real time by symbol or company name.
 *   <li>A full-height table displaying every stock with price, absolute change, percentage change,
 *       and a "Buy" action button per row.
 * </ol>
 *
 * <p>Clicking "Buy" opens a {@link PurchaseWidget} modal for the selected stock. Double-clicking
 * any row opens a {@link StockChartWidget} showing the stock's full price history.
 *
 * <p>Implements {@link GameObserver}: the table is refreshed automatically whenever the exchange
 * advances a week or a transaction is committed.
 */
public class MarketView extends VBox implements GameObserver {
  ...
  /**
   * Constructs the market view, builds the table and top container, registers as a game observer,
   * and performs an initial data refresh.
   *
   * @param marketController the controller providing stock data and purchase operations; must not
   *     be {@code null}
   */
  ...
  /**
   * Builds the market table with five columns: symbol/company, price, absolute change, percentage
   * change, and a buy-action button.
   *
   * <p>The buy-action column renders a "Buy" button per row. Clicking the button opens a {@link
   * PurchaseWidget} for that row's stock; the click event is consumed so it does not also trigger
   * the row-selection handler. Double-clicking a row opens a {@link StockChartWidget}.
   *
  ...
  private TableView<Stock> buildMarketTable() {
    ...
    TableColumnFactory.addSymbolAndCompanyColToTable(
        marketTable, Stock::getSymbol, Stock::getCompany);

    TableColumn<Stock, String> priceCol =
        TableColumnFactory.createPriceColumn("Price", Stock::getSalesPrice);
    TableColumn<Stock, String> changeCol =
        TableColumnFactory.<Stock>createColoredChangeColumn(
            "Change", s -> FormatUtil.formatPriceChange(s.getLatestPriceChange()));
    TableColumn<Stock, String> changePercentCol =
        TableColumnFactory.<Stock>createColoredChangeColumn(
   ...
   *
   * <p>The search field's text property is listened to; each change triggers {@link
   * #refreshTable(String)} to filter the market table in real time.
   *
  ...
  private VBox buildTopContainer() {
    ...
    searchField
        .textProperty()
        .addListener((observable, oldValue, searchTerm) -> refreshTable(searchTerm));
  ...
  /**
   * Filters the market table to only show stocks whose symbol or company name matches the given
   * search term.
   *
   * <p>If {@code searchTerm} is {@code null} or blank, the full list of stocks is restored.
   *
  ...
  private void refreshTable(String searchTerm) {
    ...
    marketTable.setItems(
        FXCollections.observableArrayList(marketController.findStocks(searchTerm)));
  }

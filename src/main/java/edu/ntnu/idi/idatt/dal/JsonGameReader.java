 ...
 *
 * <p>The file is expected to contain a JSON object that can be deserialised into a
 * {@link GameStateDto}. Jackson's {@link ObjectMapper} is used for deserialisation
...
public class JsonGameReader implements DataReader<GameStateDto> {
   ...
   * @return a {@link GameStateDto} representing the deserialised game state; never
   *         {@code null}
   * @throws DataAccessException if the file does not exist at {@code source}, or if the
   *                             JSON content cannot be mapped to a {@link GameStateDto}
   */
  @Override

 ...
 *
 * <p>Each factory method accepts extractor functions that are applied to the row item to produce
 * the cell text, keeping column configuration logic out of individual view classes. This class is
...
public final class TableColumnFactory {
   ...
   */
  public static <S> TableColumn<S, String> createSymbolColumn(Function<S, String> symbolExtractor) {
  ...
  /**
   * Creates a text column whose cells are coloured green or red based on whether the text contains
   * a {@code +} or {@code -} sign.
   *
   * @param <S> the type of the table row item
   * @param title the column header text
  ...
  public static <S> TableColumn<S, String> createColoredChangeColumn(
    ...
    TableColumn<S, String> col = new TableColumn<>(title);
    col.setCellValueFactory(
        data -> new SimpleStringProperty(formattedExtractor.apply(data.getValue())));
    col.setCellFactory(ViewUtility.coloredStringCellFactory());
    return col;
  }

  /**
   * Creates a column that renders differently depending on whether a row is a section header or a
   * data row.
   *
   * <p>Used by the transaction history table to insert week-header rows into the same {@link
   * TableView} as regular transaction rows.
   *

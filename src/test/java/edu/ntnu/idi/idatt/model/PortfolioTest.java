...
class PortfolioTest {
  ...
  void testAddShareSameSymbolMergesToSinglePosition() {
    ...
    assertEquals(2, portfolio.getShares().size(), "Should have exactly 2 positions (AAPL merged, GOOG separate)");
    assertEquals(0, new BigDecimal("11").compareTo(portfolio.getShares("APPL").getFirst().getQuantity()), "Quantity should be 1 + 10 = 11");
  }
  ...
  void testReduceSharePartial() {
    ...
    assertEquals(0, new BigDecimal("7").compareTo(remaining),
        "Remaining quantity should be 7 (11 - 4)");
  }

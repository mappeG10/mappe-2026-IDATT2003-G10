 ...
 *
 * <p>Includes convenience methods for sign-based CSS styling, coloured table cells, rounded-corner
 * clipping, double-click row handlers, and modal alert dialogs. This class is not instantiable; all
 * methods are static.
 */
public class ViewUtility {
  ...
  /**
   * Applies a {@code text-positive} or {@code text-negative} CSS class to a labeled node based on
   * the sign of the given value.
   *
   * <p>Any previously applied sign class is removed before the new one is added. If the value is
   * exactly zero, no sign class is applied.
   *
   * @param node the labeled node to style; must not be {@code null}
   * @param value the value whose sign determines the applied CSS class; must not be {@code null}
   */
  ...
  /**
   * Returns a cell factory that colours a string cell green if its text contains {@code +} or red
   * if it contains {@code -}.
   *
   * <p>The factory applies the {@code text-positive} or {@code text-negative} CSS class
   * respectively, and clears both classes when the cell is empty or its value changes.
   *
   * @param <S> the type of the table row item
   * @return a {@link Callback} suitable for use with {@link TableColumn#setCellFactory(Callback)}
   */
  public static <S>
      Callback<TableColumn<S, String>, TableCell<S, String>> coloredStringCellFactory() {
    return col ->
        new TableCell<>() {
          /** {@inheritDoc} */
          @Override
   ...
   *
   * <p>Listeners are attached to the region's width and height properties so the clip rectangle
   * resizes automatically when the region's dimensions change.
   *
   ...
   *
   * <p>The callback receives the row's item and the owning window, so it can open a detail dialog
   * anchored to the correct parent.
   *
   * @param <T> the type of the table row item
   * @param onDoubleClick the callback to invoke on a primary-button double-click; receives the row
   *     item and the owning {@link Window}
  ...
  public static void showErrorAlert(String title, String message) {
    Window owner =
        Window.getWindows().stream().filter(Window::isShowing).reduce((a, b) -> b).orElse(null);

...
class StockTest {
  ...
  void setUp() {
    prices =
        new ArrayList<>(
            List.of(new BigDecimal("182.50"), new BigDecimal("183.75"), new BigDecimal("181.20")));
  ...
  void testConstructorWithValidArguments() {
    ...
    assertEquals(symbol, stock.getSymbol(), "Constructor didn't initialise symbol correctly");
    assertEquals(
        company, stock.getCompany(), "Constructor didn't initialise company name correctly");
    assertEquals(
        prices.getLast(),
        stock.getSalesPrice(),
  ...
  void testGetHistoricalPrices() {
    ...
    assertEquals(
        0,
        salesPrices.getFirst().compareTo(prices.getFirst()),
        "Should contain the price from initialization list");
    assertEquals(
        0,
        salesPrices.get(1).compareTo(prices.get(1)),
        "Should contain the price from initialization list");
    assertEquals(
        0,
        salesPrices.get(2).compareTo(prices.get(2)),
        "Should contain the price from initialization list");
  }
  ...
  void testGetSalesPriceWithEmptyPricesListThrowsException() {
    ...
    assertThrows(
        ...
        stockWithEmptyPrices::getSalesPrice,
  ...
  void testGetLatestPriceChangeDecrease() {
    ...
    assertEquals(
        0,
        expectedPriceChange.compareTo(actualPriceChange),
  ...
  void testGetLatestPriceChangeThrowsWhenPricesListIsEmpty() {
    ...
    assertThrows(
        IllegalStateException.class,
        stockWithNoPrices::getLatestPriceChange,
  ...
  void testGetLatestPriceChangeReturnsZeroWhenPricesHasOnePrice() {
    ...
    assertEquals(
        0,
        BigDecimal.ZERO.compareTo(stockWithOnePrice.getLatestPriceChange()),
  ...
  void testGetHighestAndLowestPriceOnEmptyListThrowsException() {
    ...
    assertThrows(
        IllegalStateException.class,
        stockWithEmptyPrices::getHighestPrice,
        "GetHighestPrice should throw exception when prices list is empty");
    assertThrows(
        IllegalStateException.class,
        stockWithEmptyPrices::getLowestPrice,
  ...
  void testGetLatestPriceChangePercent() {
    ...
    assertEquals(
        0,
        expectedPercent.compareTo(stock.getLatestPriceChangePercent()),
  ...
  void testGetLatestPriceChangePercentReturnsZeroWithOnePrice() {
    ...
    assertEquals(
        0,
        BigDecimal.ZERO.compareTo(stockWithOnePrice.getLatestPriceChangePercent()),
  ...
  void testGetLatestPriceChangePercentThrowsWhenPricesListIsEmpty() {
    ...
    assertThrows(
        IllegalStateException.class,
        stockWithNoPrices::getLatestPriceChangePercent,
  ...
  void testHasPriceChangeReturnsTrueWithMultiplePrices() {
    List<BigDecimal> listWithTwoPrices =
        new ArrayList<>(List.of(new BigDecimal("180.20"), new BigDecimal("200.20")));
